...
数学/C 对照验证、第二阶段的原结构全局工作空间分析、第三阶段的正常装配
模式连续伸缩研究，以及面向 150 mm 台阶的任务级动力学决策门；不涉及杆长
优化、完整跳跃仿真、MuJoCo、强化学习或 ROS。
...
完整 ENGINEERING DECISION REVIEW 见
[`docs/research/2026-08-23-stair-decision-gate.md`](docs/research/2026-08-23-stair-decision-gate.md)。
这一部分不是完整动力学仿真，而是用常力点质量能量关系和 Phase 3 的精确
传动比，判断任务数量级是否已经否定原杆长或 EduLite 候选，并找出最值得
实测的变量。

![150 mm stair decision gate](artifacts/stair_decision_gate/stair_decision_gate.png)

左上图把三个概念分开：完全无接触飞越要求轮轴上升 150 mm；若离地后从
120 mm 收腿至 70 mm，并假设其中 90% 转化为轮轴相对质心的上移，则整机
质心上升基准约为 105 mm；借助台阶上边缘捕获可给出 79 mm 的乐观几何
下界，但尚未证明这种接触稳定可靠。收腿越边后还必须及时重新伸腿，才能为
落地恢复缓冲行程；当前模型没有证明这三段时序可完成。

对机器人实际意味着：26 mm 半径的轮子不能从地面开始就接触台阶上边缘。
轮心至少先上升 `150-26=124 mm`，才可能在立面外侧接触上角。轮胎可以更早
碰到竖直立面，但利用立面摩擦上举需要可控法向力，当前没有证据。因此值得
优先研究前向跃阶、蹬伸后飞行收轮和晚期边缘捕获，而不是让 QD4310 单独静态
滚上台阶。2.5 kg 两轮均载时，最不利边缘重力矩约 `0.319 N m/轮`，已经略
高于 QD4310 标称 `0.3 N m` 峰值。

右上图显示能量随最终质量线性增加。对 2.5 kg、50 mm 蹬伸：

| 理想筛选事件 | 机械功/能量 | 最大单关节扭矩 | 最大关节速度 | 峰值四关节机械功率 |
|---|---:|---:|---:|---:|
| 完全飞越 150 mm | 3.68 J | 1.24 N m | 215 rpm | 103 W |
| 90% 收腿有效、质心升 105 mm | 2.57 J | 0.87 N m | 160 rpm | 53.5 W |
| 150 mm 下落、50 mm 压缩 | 4.90 J | 1.66 N m | 264 rpm | 168 W 回馈 |
| 150 mm 下落、20 mm 压缩 | 4.17 J | 3.44 N m | 248 rpm | 357 W 回馈 |

对机器人实际意味着：上台阶没有出现明显违反 EduLite 48 V 官方曲线的数量级
矛盾，所以 EduLite 暂时保留；但这只是常力、无损失、50/50 均载且无冲击
尖峰的筛选，不能批准批量采购。下台阶更接近风险边界，真实压缩行程从
50 mm 缩短到 20 mm 会把理想峰值力、扭矩和回馈功率大幅推高。

左下图将 2.5 kg 理想事件放到 EduLite 05 的 torque-speed 平面。黑线是官方
48 V 手册曲线的近似数字化；灰线只是 24 V 下速度轴按电压缩放的敏感性假设，
不是厂家曲线。48 V 下候选没有被否定；24 V 假设则显示完全飞越和落地速度
可能超出范围，因此必须索取或实测目标电压下的完整曲线。

右下图说明 CAD 行程为什么现在具有决策价值：更长压缩虽然要吸收更多重力
继续做的功，却显著降低峰值关节扭矩和回馈功率。下一次 CAD 工作必须导入
EduLite STEP 后确认真实 70--120 mm，而不能只扫描仍装 4010 的旧总装。

候选六电机的官方标称质量合计 `1.222 kg`，已经占 2.0 kg 整机目标的
61.1%、占 2.5 kg 的 48.9%，且未含支架、电池和电子件。EduLite 性能基于
48 V，而 QD4310 只允许 7--26 V；落地阻尼又会向母线回馈能量。实际系统
必须解决质量预算、48 V 主母线/轮端降压和回馈泄放，不能只比较峰值扭矩。
固定版本执行器证据见
[`reference/actuator_evidence.md`](reference/actuator_evidence.md)。

上游 LQR 也不能数值照搬：其 `sys_calc.m` 建模轮径为 20 mm，而控制代码使用
26 mm，质量参数更不代表新的 2.0--2.5 kg 整机。当前可以继承状态定义和控制
架构，最终增益必须按实际质量、惯量、轮径和执行器重新推导。
...
下一个工程工作包定义为 `WP-STAIR-REALITY-GATE`。先实测目标建筑楼梯的
高度范围、踏面深度、鼻口、表面和平台空间；150 mm 高度本身不足以定义连续
楼梯动作。然后导入官方 EduLite STEP，保持原杆长和主动轴线，扫描
70--120 mm 的碰撞、最小间隙、硬限位和线束空间，同时建立完整
2.0/2.3/2.5 kg 质量预算。只购买一个 EduLite 做 48 V 短时 torque-speed、
反驱和 2--5 J 回馈母线试验。通过这些门之后，才批准其余电机采购和
sagittal-plane/MuJoCo 动力学建模。
