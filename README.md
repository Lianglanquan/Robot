...
装配模式连续伸缩研究、真实机械包络验证和 EduLite 主动关节最小补丁；不涉及杆长优化、
...
都加入了明确标准螺钉。审计还实际抓出了两个问题：第一版底脚与底板都是通孔，
没有锁紧位置；简单镜像的右支架后部 4×M3 与真实电机相差约 5.36 mm。现在
底脚使用 M3 攻牙和 M3x10，右侧按官方 STEP 的真实孔钟向重切；左右底板孔误差
为零，四台电机后孔误差仅为数值舍入量级。

加入 24 颗随主动杆运动的 M4 输出螺钉后，70--120 mm 的 51 个姿态仍全部
CLEAR，最小间隙保持 `0.9151696 mm @ 70 mm`。所有新增结构件和简化螺钉均为
有效单实体。当前整机 STEP 是左右完整运动骨架，不把已知冲突的旧电池/NanoPi
支架或失去支撑的板卡悬空放回；下一步是以该骨架为边界重新布置内部器件。
