...
数学/C 对照验证，以及第二阶段的原结构工作空间、Jacobian 奇异性和归一化
力/速度传递分析；不涉及杆长优化、EduLite 参数、MuJoCo、强化学习或 ROS。
...
58.91%、可用 31.68%、不建议 9.41%。右侧 XY 图先画不建议、最后画推荐，
因此表示某个足端位置是否至少存在良好内部构型；左侧关节空间图才是选择
具体 `phi1/phi4` 时应使用的风险图。
...
第二阶段已经建立可复现的原结构基线。下一步可以在不改数学定义的前提下
加入真实关节限位、杆件/机身碰撞和执行器扭矩-转速包络，再分析跳跃推力、
落地回馈扭矩和台阶动作路径。当前阶段没有改变杆长，也没有加入 EduLite
参数或把控制层保护误当作机械结构约束。
